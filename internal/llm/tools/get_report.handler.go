...
type GetReportInput struct {
	...
	Period string `json:"period,omitempty" jsonschema:"Financial report period: Q (Quarter) or Y (Year). Default is Q"`
}
...
func HandleGetReport(ctx context.Context, _ Deps, input GetReportInput) (any, error) {
	if input.Symbol == "" {
	...
	httpReq, err := http.NewRequestWithContext(ctx, "POST", common.GRAPHQL_URL, bytes.NewBuffer(payload))
	if err != nil {
		return nil, err
	}
