...
func (s *Server) GetNewsHandler(w http.ResponseWriter, r *http.Request) {
	...
	latestNews, err := s.db.GetLatestNews(r.Context(), currentDate)
	...
	if err != nil || len(latestNews) == 0 {
		...
		tavilyNews, tErr := s.tavily.SearchWeb(r.Context(), "Tin tức thị trường chứng khoán Việt Nam hôm nay", common.NEWS_DOMAINS)
		...
		for _, article := range tavilyNews {
			saved, sErr := s.db.SaveNews(r.Context(), database.SaveNewsParams{
				Title:       article.Title,
				Url:         article.URL,
