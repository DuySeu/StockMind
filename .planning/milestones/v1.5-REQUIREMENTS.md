...
- [x] **UPLOAD-01**: User upload file từ frontend, hỗ trợ định dạng: PDF, DOCX, MD, TXT
- [x] **UPLOAD-02**: Frontend validate file trước khi upload: đúng định dạng, không quá 10MB
- [x] **UPLOAD-03**: User chọn chunking strategy khi upload: Smart Split (recursive), Fixed Size, By Paragraph, By Topic (semantic)
- [x] **UPLOAD-04**: Upload endpoint trả về HTTP 202 Accepted ngay lập tức với `{id, status: "pending"}` — không block chờ processing
- [x] **UPLOAD-05**: File được lưu tạm thời, record được tạo trong DB với `status = pending`

### Document Processing

- [x] **PROC-01**: Background worker nhận job và parse text từ file: PDF (pdfcpu), DOCX (stdlib zip+xml), MD (strip formatting), TXT (raw)
- [x] **PROC-02**: Worker validate chất lượng text sau parsing: nếu < 100 ký tự có nghĩa → mark `failed` với thông báo rõ ràng
...
- [x] **PROC-08**: Worker pool có tối đa 2 concurrent jobs; wired vào server shutdown context (graceful stop)
- [x] **PROC-09**: Job queue là buffered channel capacity 10; nếu đầy, upload nhận 503 Service Unavailable

### Document Management

- [x] **DOC-01**: `GET /api/documents` trả về danh sách tất cả documents với metadata: id, name, file_type, status, chunk_count, created_at
- [x] **DOC-02**: `GET /api/documents/:id` trả về chi tiết một document kể cả error_msg nếu failed
- [x] **DOC-03**: `DELETE /api/documents/:id` xóa document record khỏi DB VÀ xóa tất cả vectors liên quan khỏi Qdrant

### RAG Retrieval (MCP Tool)

- [x] **RAG-01**: MCP tool `retrieve_knowledge` được đăng ký trong MCP engine, nhận tham số `query: string`
- [x] **RAG-02**: Tool embed query dùng cùng model (`nomic-ai/nomic-embed-text-v1.5`) để đảm bảo tương thích vector space
- [x] **RAG-03**: Tool tìm kiếm Qdrant top-5 với score threshold ≥ 0.70 (cosine similarity)
- [x] **RAG-04**: Nếu không có kết quả vượt threshold, tool trả về "No relevant information found in knowledge base"
- [x] **RAG-05**: Tool description đủ cụ thể để LLM phân biệt: dùng cho định nghĩa/khái niệm/tài liệu, KHÔNG dùng cho giá realtime/news/tính toán
- [x] **RAG-06**: LLM tự quyết định intent-based: dùng `retrieve_knowledge` hay các agent tools khác

### Frontend UI

- [x] **UI-01**: Trang Document Management (`/documents`) có form upload với: file picker, strategy dropdown, nút upload
- [x] **UI-02**: Sau upload thành công, document xuất hiện ngay trong danh sách với badge "Processing"
- [x] **UI-03**: Danh sách documents hiển thị: tên file, loại file (icon), ngày upload, status badge (Processing/Ready/Failed), số chunks
- [x] **UI-04**: Frontend poll `GET /api/documents` mỗi 3 giây khi có document đang ở trạng thái "processing"
- [x] **UI-05**: Document ở trạng thái "Failed" hiển thị tooltip/expand với lý do lỗi
- [x] **UI-06**: User có thể xóa document (có confirmation dialog); xóa xong cập nhật danh sách
- [x] **UI-07**: Empty state khi chưa có document: "Upload tài liệu để tăng cường khả năng trả lời của AI"
...
| UPLOAD-01 | Phase 4 | Complete |
| UPLOAD-02 | Phase 6 | Complete |
| UPLOAD-03 | Phase 4 + Phase 6 | Complete |
| UPLOAD-04 | Phase 4 | Complete |
| UPLOAD-05 | Phase 4 | Complete |
